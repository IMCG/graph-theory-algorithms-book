...
\centering
\index{Kneser graph}
\input{image/graph-algorithms_5-2-Kneser-graph.tex}
...
Johnson's\index{Johnson's algorithm} algorithms. See
...
\item Consider the world map of major capital cities in
  ...
  \item Table~\ref{tab:graph_algorithms:capital_cities_distance} lists
    ...
    run Dijkstra's\index{Dijkstra's algorithm} algorithm over this
  ...
  \item Repeat the previous exercise, but use the
    ...
    Dijkstra's\index{Dijkstra's algorithm} algorithm. Compare the
...
\input{image/graph-algorithms_worldmap-capital-cities.tex}
...
\index{Washington DC}\index{USA}
{\tiny
\input{data/graph-algorithms_capital-cities-distance.tex}
}
...
\item Binary search is a much faster searching algorithm than linear
  search. The binary search algorithm assumes that its input list is
  ordered in some manner. For simplicity, we assume that the input
  list $L$ consists of positive integers. The main idea of binary
  search is to partition $L$ into two halves: the left half and the
  right half. Our task now is to determine whether the object $E$ of
  interest is in the left half or the right half, and apply binary
  search recursively to the half in which $E$ is
  located. Algorithm~\ref{alg:graph_algorithms:binary_search} provides
