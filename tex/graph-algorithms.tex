...
novel \emph{Gulliver's Travels}\index{Gulliver's Travels} first
published in~1726, Jonathan Swift\index{Swift, Jonathan} used the
terms big-\index{big-endian} and little-endian\index{little-endian} in
...
\emph{distance}\index{distance} from $u$ and $v$, and will be
...
\emph{diameter}\index{diameter} of a graph $G = (V,E)$ is defined as
\[
...
algorithm\index{Dijkstra's algorithm} can be used to find a shortest
...
\item Various efficient search techniques exist that cater for special
  ...
  \item Investigate an algorithm for and time complexity of trie
    search~(see section~6.3 in Knuth~\cite{Knuth1998}).
  ...
  \item Hashing techniques can result in searches that run in $O(1)$
    ...
    hashing can be used to speed up searches~(see section~6.4 in
    Knuth~\cite{Knuth1998}). For further information on hashing and its
    application to cryptology, see
...
\item In addition to bubble and selection sort, other algorithms exist
  ...
  algorithms. Chapter~5 in Knuth~\cite{Knuth1998} describes various
  ...
  \item Investigate and provide pseudocode for insertion sort and
    ...
    and selection sort. For more information, see section~5.2.1 in
    Knuth~\cite{Knuth1998} or pages~98--104 in
    Sedgewick~\cite{Sedgewick1990}.

  \item Shellsort is a variation on insertion sort that can speed up
    the runtime of insertion sort~(see pages~83--95 in
    Knuth~\cite{Knuth1998} or pages~107--111 in
    Sedgewick~\cite{Sedgewick1990}). Describe and provide pseudocode
    for shellsort. Compare the time complexity of shellsort with that
    of insertion sort. In what ways is shellsort different from or
  ...
  \item The quicksort algorithm due to C.~A.~R. Hoare~\cite{Hoare1962}
    ...
    sorting algorithms covered in this chapter. For more information
    on quicksort, see pages~113--122 in Knuth~\cite{Knuth1998} or
    chapter~9 in Sedgewick~\cite{Sedgewick1990}.
  \end{enumerate}
