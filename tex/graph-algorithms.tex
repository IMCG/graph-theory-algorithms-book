...
$M = [a_{ij}]$ be the adjacency matrix\index{matrix!adjacency} of
$G$. Consider the upper triangle\index{matrix!upper triangle} of $M$,
excluding the main diagonal\index{matrix!main diagonal}, and write
that upper triangle as the bit vector\index{bit vector}
\[
v
=
...
presents BFS\index{BFS} trees\index{breadth-first search!tree}
...
\centering
\index{knight's tour}
\input{image/graph-algorithms_one-knight-tour.tex}
...
exists, is called a \emph{knight's tour}\index{knight's tour}. How do
...
tour\index{knight's tour} is an example of depth-first
...
Loebbing and Wegener~\cite{LoebbingWegener1996} announced in 1996 that
this number is 33,439,123,484,294. The answer was later corrected by
McKay~\cite{McKay1997} to be 13,267,364,410,532.
See~\cite{ElkiesStanley2003} for a discussion of the knight's tour and
...
shows the DFS\index{DFS} trees\index{depth-first search!tree}
...
DFS\index{DFS} can be analyzed using an argument similar to that in
...
stack\index{stack} implementation.
...
nonnegative edge weights\index{weight!nonnegative} $w(e)$ for each
edge $e \in E$. The \emph{length}\index{path!length} or
...
distances\index{distance}, a $v$-$w$ path that realizes $d(v,w)$ is
sometimes called a \emph{shortest path}\index{path!shortest} from $v$
to $w$.
...
$s$ of a cycle\index{cycle} such that $s < 0$.
...
nonnegative edge weights\index{weight!nonnegative}. Imagine that the
vertices of a weighted graph represent cities and edge weights
represent distances\index{distance} between pairs of cities connected
by a direct road. Dijkstra's algorithm\index{Dijkstra's algorithm} can
be used to find a shortest route from a fixed city to any other city.
...
\begin{example}
Apply Dijkstra's algorithm\index{Dijkstra's algorithm} to the graph in
...
algorithm\index{Dijkstra's algorithm}, the distance and parent vertex
...
Dijkstra's algorithm\index{Dijkstra's algorithm} is an example of a
\emph{greedy algorithm}\index{greedy algorithm}. Whenever it tries to
find the next vertex, it chooses only that vertex that minimizes the
total weight\index{weight!minimum} so far. Greedy
algorithms\index{greedy algorithm} may not produce the best possible
result. However, as the following theorem shows, Dijkstra's
algorithm\index{Dijkstra's algorithm} does indeed produce shortest
...
algorithm\index{Dijkstra's algorithm} is applied to $G$ with source
...
algorithm\index{Dijkstra's algorithm}? The time complexity of
...
\noindent
A disadvantage of Dijkstra's\index{Dijkstra's algorithm}
...
the ``greedy''\index{greedy algorithm} approach that Dijkstra's
algorithm\index{Dijkstra's algorithm} takes, i.e. searching for the
...
Dijkstra's\index{Dijkstra's algorithm}
...
Floyd-Warshall algorithm\index{Floyd-Warshall algorithm}, is an
algorithm for finding shortest paths\index{path!shortest} in a
weighted, directed graph. Like the Bellman-Ford
algorithm\index{Bellman-Ford algorithm}, it allows for negative edge
weights\index{weight!negative} and detects a negative weight
cycle\index{cycle!negative} if one exists. Assuming that there are no
negative weight cycles, a single execution of the FRW\index{FRW}
...
thesis of Esko Nuutila~\cite{Nuutila1995}\index{Nuutila, Esko}. The
...
algorithm\index{Kleene's algorithm}.
...
main insight of Johnson's algorithm\index{Johnson's algorithm} is to
...
cycles\index{cycle!negative}. Next, we reweight\index{reweight} edges
