...
either the distance from one city to another or the cost to travel
from one city to another.

Shortest path algorithms\index{shortest path algorithm} are some of
the most important algorithms in algorithmic graph theory. We first
examine several common graph traversal algorithms and some basic data
structures\index{data structure} underlying these algorithms. A data
structure is a combination of methods for structuring a collection of
data~(e.g. vertices and edges) and protocols for accessing the
data. Subsequent shortest path algorithms rely in one way or another
on graph traversal techniques and basic data structures for organizing
...
adjacency list\index{adjacency list} representation that can result in
less storage space requirement than the adjacency
matrix\index{adjacency matrix} representation. The
\texttt{graph6}\index{\texttt{graph6}} format discussed in
...
Define the adjacency lists\index{adjacency list} of a graph as
...
\emph{adjacency list}\index{adjacency list} of $v_i$. Then
...
each adjacency list\index{adjacency list} $L_i$ contains $s_i$
...
representation. An adjacency list\index{adjacency list} is explicit
...
\begin{figure}[!htbp]
\centering
\input{image/graph-algorithms_graph-adjacency-lists.tex}
...
matrix\index{adjacency matrix} representation as unsuitable. For a
sparse graph, an adjacency list\index{adjacency list} representation
...
The graph formats \texttt{graph6}\index{\texttt{graph6}} and
\texttt{sparse6}\index{\texttt{sparse6}} were developed by Brendan
McKay\index{McKay, Brendan}~\cite{McKay2010} at The Australian
...
\texttt{graph6} and \texttt{sparse6} are those ASCII characters with
decimal codes from 63 to 126, inclusive, as shown in
Table~\ref{tab:graph_algorithms:graph6_sparse6_ASCII_printable_characters}.
This section shall only cover the \texttt{graph6} format. For full
specification on both of the \texttt{graph6} and \texttt{sparse6}
formats, see McKay~\cite{McKay2010}.

\begin{table}[!htbp]
\centering
\input{data/graph-algorithms_graph6-sparse6-ASCII-printable-characters.tex}
...
Before discussing how \texttt{graph6}\index{\texttt{graph6}} and
\texttt{sparse6}\index{\texttt{sparse6}} represent graphs using
printable ASCII\index{ASCII} characters, we first present encoding
schemes used by these two formats. A
\emph{bit vector}\index{bit vector} is, as its name suggests, a
vector whose elements are 1's and 0's. It can be represented as a list
of bits\index{bit}, e.g. \verb!E! can be represented as the ASCII bit
vector $[
...
\emph{most significant bit}\index{most significant bit} of a bit
vector $v$ is the bit position with the largest value among all the
bit positions in $v$. Similarly, the
\emph{least significant bit}\index{least significant bit} is the bit
...
as \emph{endianness}\index{endianness}. Processing $v$ in
\emph{big-endian}\index{big-endian} order means that we first process
...
bit of $v$. \emph{Little-endian}\index{little-endian} order means that
...
significant bit. In big-endian order, the ASCII\index{ASCII} binary
...
\begin{table}[!htbp]
\centering
\input{data/graph-algorithms_big-endian-ASCII-binary-E.tex}
...
\begin{table}[!htbp]
\centering
\input{data/graph-algorithms_little-endian-ASCII-binary-E.tex}
\caption{Little-endian order of the ASCII binary code of \texttt{E}.}
\label{tab:graph_algorithms:little_endian_ASCII_binary_E}
\end{table}

In \texttt{graph6}\index{\texttt{graph6}} and
\texttt{sparse6}\index{\texttt{sparse6}} formats, the length of a bit
vector\index{bit vector} must be a multiple of 6. Suppose $v$ is a bit
vector of length $k$ such that $6 \nmid k$. To transform $v$ into a
bit vector having length a multiple of 6, let $r = k \mod 6$ be the
remainder\index{remainder} upon dividing $k$ by 6, and pad $6 - r$
zeros\index{zero padding} to the right of $v$.
...
\subsubsection{The \texttt{graph6} format}
\index{\texttt{graph6}}

The \texttt{graph6}\index{\texttt{graph6}} format is used to represent
simple, undirected graphs of order from $0$ to $2^{36} - 1$,
inclusive. Let $G$ be a simple, undirected graph of order
$0 \leq n \leq 2^{36} - 1$. If $n = 0$, then $G$ is represented in
\texttt{graph6} format as ``\verb!?!''. Suppose $n > 0$. Let
$M = [a_{ij}]$ be the adjacency matrix\index{adjacency matrix} of
$G$. Consider the upper triangle\index{upper triangle} of $M$,
excluding the main diagonal\index{main diagonal}, and write that upper
triangle as the bit vector\index{bit vector}
\[
v
=
...
column $i$ of $M$. Then the \texttt{graph6}\index{\texttt{graph6}}
representation of $G$ is $N(n) R(v)$, where $R(v)$ and $N(n)$ are as
...
\input{algorithm/graph-algorithms_breadth-first-search-template.tex}
...
\begin{figure}[!htbp]
\centering
\input{image/graph-algorithms_breadth-first-search-undirected.tex}
...
\begin{algorithm}[!htpb]
\input{algorithm/graph-algorithms_depth-first-search-template.tex}
...
\begin{figure}[!htbp]
\centering
\input{image/graph-algorithms_depth-first-search-undirected.tex}
...
$c \assign 1$ to mean that we are starting our exploration at $s$,
...
\begin{algorithm}[!htpb]
\input{algorithm/graph-algorithms_graph-connectivity.tex}
...
known as a \emph{metric function}\index{metric function}. Intuitively,
the distance between two physical locations is greater than zero. When
these two locations coincide, i.e. they are one and the same location,
the distance separating them is zero. Regardless of whether we are
measuring the distance from location $a$ to $b$ or from $b$ to $a$, we
would obtain the same distance. Imagine now a third location $c$. The
distance from $a$ to $b$ plus the distance from $b$ to $c$ is greater
than or equal to the distance from $a$ to $c$. The latter principle is
known as the
\emph{triangle inequality}\index{triangle inequality}. In summary,
given three vertices $u,v,w$ in a graph $G$, the distance function $d$
on $G$ satisfies the following property.
...
$w: E \to \R$. Define a distance function $d: V \times V \to \R$
given by
\[
d(u,v)
=
...
where the word ``metric'' refers to the distance function $d$. Any
graphs we consider are assumed to have finite sets of vertices. For
this reason, $(V,d)$ is also known as a
\emph{finite metric space}\index{metric space!finite}. The distance
matrix $D = [d(v_i, v_j)]$ of a connected graph is the distance matrix
of its finite metric space. The topic of metric space is covered in
further details in topology texts such as Runde~\cite{Runde2005} and
Shirali and Vasudeva~\cite{ShiraliVasudeva2006}.
...
\begin{algorithm}[!htpb]
\input{algorithm/graph-algorithms_generic-shortest-path-algorithm.tex}
...
\emph{weight-correcting}~\cite{GalloPallottino1986}. A weight-setting
...
\begin{algorithm}[!htpb]
\input{algorithm/graph-algorithms_dijkstra-general.tex}
...
\begin{figure}[!htbp]
\centering
\input{image/graph-algorithms_Dijkstra-algorithm-digraph.tex}
...
\begin{table}[!htbp]
\centering
\input{data/graph-algorithms_working-through-Dijkstra-algorithm.tex}
...
\begin{table}[!htbp]
\centering
\input{data/graph-algorithms_worst-case-time-complexity-Dijkstra.tex}
...
\begin{algorithm}[!htpb]
\input{algorithm/graph-algorithms_Bellman-Ford.tex}
...
\begin{algorithm}[!htpb]
\input{algorithm/graph-algorithms_Bellman-Ford-redundant-updates.tex}
...
\begin{algorithm}[!htpb]
\input{algorithm/graph-algorithms_Floy-Roy-Warshall.tex}
...
\begin{figure}[!htbp]
\centering
\input{image/graph-algorithms_Floyd-Roy-Warshall-demo.tex}
...
\begin{figure}[!htbp]
\centering
\input{image/graph-algorithms_another-Floyd-Roy-Warshall-demo.tex}
...
\begin{algorithm}[!htpb]
\input{algorithm/graph-algorithms_Floy-Roy-Warshall-transitive-closure.tex}
...
Let $G = (V,E)$ be a sparse\index{sparse graph} digraph with edge
weights but no negative cycles.
\emph{Johnson's algorithm}\index{Johnson's algorithm}~\cite{Johnson1977}
finds a shortest path between each pair of vertices in $G$. First
published in 1977, the main insight of Johnson's algorithm is to
combine the technique of edge reweighting\index{reweight} with the
Bellman-Ford\index{Bellman-Ford algorithm} and
Dijkstra's\index{Dijkstra's algorithm} algorithms. The Bellman-Ford
algorithm is first used to ensure that $G$ has no negative
cycles. Next, we reweight edges in such a manner as to preserve
shortest paths. The final stage makes use of Dijkstra's algorithm for
computing shortest paths between all vertex pairs. Pseudocode for
Johnson's algorithm is presented in
Algorithm~\ref{alg:graph_algorithms:Johnson_algorithm}. With a
Fibonacci heap\index{Fibonacci heap} implementation of the
minimum-priority queue\index{minimum-priority queue}, the time
complexity for sparse graphs is $O(|V|^2 \log |V| + |V| \cdot |E|)$,
...
%%
\begin{enumerate}
\item The reweighted\index{reweight} edges preserve shortest
  paths. That is, let $p$ be a $u$-$v$ path for $u,v \in V$. Then $p$
  is a shortest weighted path\index{weighted path}\index{shortest path}
  using weight function $w$ if and only if $p$ is also a shortest
  weighted path using weight function $\hat{w}$.

\item The reweight function $\hat{w}$ produces nonnegative weights. In
  other words, if $u,v \in V$ then $\hat{w}(uv) \geq 0$.
\end{enumerate}
%%
...
\input{algorithm/graph-algorithms_Johnson-algorithm.tex}
...
\textbf{Reweighting preserves shortest paths.}\index{reweight}
...
%%
\begin{enumerate}
\item The path $p$ is a shortest $v_0$-$v_k$ path with weight function
  $w$ if and only if it is a shortest $v_0$-$v_k$ path with weight
  function $\hat{w}$.

\item The graph $G$ has a negative cycle using weight function $w$ if
  and only if $G$ has a negative cycle using $\hat{w}$.

\item If $G$ has no negative cycles, then $\hat{w}(uv) \geq 0$ for all
  $uv \in E$.
\end{enumerate}
\end{lemma}
...
To prove part~2, consider any cycle $c: v_0, v_1, \dots, v_k$ where
$v_0 = v_k$. Using the proof of part~1, we have
%%
\begin{align*}
\hat{w}(c)
&=
...
incoming edges and any path in $G'$ that contains $s$ has $s$ has the
source vertex. Thus $G'$ has no negative cycles if and only if $G$ has
no negative cycles. Define the function $h: V \to \R$ by
...
\begin{figure}[!htbp]
\centering
\input{image/graph-algorithms_Dijkstra-directed-house-graph.tex}
