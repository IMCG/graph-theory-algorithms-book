...
is a rooted binary tree $T = (V,E)$ having weighted vertices
$\kappa: V \to \R$. The weight of each vertex $v$ is referred to as
its key, denoted $\kappa_v$. Each vertex $v$ of $T$ satisfies the
following properties:
%%
\begin{itemize}
\item The left subtree of $v$ contains only vertices whose keys are at
  most $\kappa_v$. That is, if $u$ is a vertex in the left subtree of
  $v$, then $\kappa_u \leq \kappa_v$.

\item The right subtree of $v$ contains only vertices whose keys are
  at least $\kappa_v$. In other words, any vertex $u$ in the right
  subtree of $v$ satisfies $\kappa_v \leq \kappa_u$.

\item Both the left and right subtrees of $v$ must also be binary
  search trees.
\end{itemize}
%%
...
the input BST is empty then \texttt{NULL} is returned.
...
$T$ with minimum key can be found by repeatedly traversing left
subtrees. When we have reached the left-most vertex $v$ of $T$,
querying for the left subtree of $v$ should return \texttt{NULL}. At
this point, we conclude that $v$ is a vertex with minimum key. Each
query for the left subtree moves us one level down $T$, resulting in a
worst-case runtime of $O(h)$ with $h$ being the height of $T$.
...
\item Explain any differences between the binary
  heap-order\index{binary heap!heap-order property} property, the
  binomial heap-order\index{binomial heap!heap-order property}
