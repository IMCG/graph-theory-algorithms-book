...
\item The upper bound in
  ...
  the recurrence relation $N(h) > N(h - 1) + N(h - 2)$.
  %%
  \begin{enumerate}[(a)]
  ...
  \item Assume for induction that
    ...
    solution to the recurrence relation $c^{h-1} + c^{h-2}$ and that
    \[
    N(h)
    >
  ...
  \item Use the previous two parts to show that
    ...
    \]
    where $\varphi = (1 + \sqrt{5}) / 2$ is the golden ratio and $n$
    counts the number of internal vertices of an AVL tree of height
    $h$.
  \end{enumerate}
\end{problem}
