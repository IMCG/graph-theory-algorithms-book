...
%%
\begin{enumerate}
\item Height-balanced: $T_v$ is height-balanced so no need to do
  anything further. A simple way to detect this is to consider the
  subtree $S$ rooted at $u$, the parent of $v$. If $S$ is a complete
  binary tree, then no height adjustment need to take place for
  vertices in $P_u$, hence $T_v$ is an AVL tree. Otherwise we perform
  any necessary height adjustment for vertices in $P_u$, starting from
  $u_k = u$ and working our way up to the root $r = u_0$. After
  adjusting the height of $u_i$, we test to see whether $u_i$~(with
  its new height) is height-balanced. If each of the $u_i$ with their
  new heights are height-balanced, then $T_v$ is height-balanced.

\item Height-unbalanced: During the height adjustment phase, it may
  happen that some $u_j$ with its new height is not
  height-balanced. Among all such height-unbalanced vertices, let
  $u_\ell$ be the first height-unbalanced vertex detected during the
  process of height adjustment starting from $u_k = u$ and going up
  towards $r = u_0$. We need to rebalance the subtree rooted at
  $u_\ell$ and perform another round of height adjustment.
\end{enumerate}
