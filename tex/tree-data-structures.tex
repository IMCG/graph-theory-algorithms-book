...
priority of each element of $Q$ belongs to the same totally ordered
set $X$, we use the total order defined on $X$ to compare the
priorities of the queue elements. For example, the set $\Z$ of
integers is totally ordered by the ``less than or equal to''
relation. If the priority of each $e \in Q$ is an element of $\Z$, we
use the latter relation to compare the priorities of elements of
$Q$. In the case of an ordinary queue, the priority of each queue
element is its position index.
...
priority. Let $p_i$ be the priority assigned to element $e_i$ of
$Q$. Then $p_{\min}$ is the lowest priority if $p_{\min} \leq p$ for
any element priority $p$. The element with corresponding priority
$p_{\min}$ is the minimum priority element. Based upon the notion of
priority comparison, we define two operations on a priority queue:
%%
\begin{itemize}
...
each element $e \in L$ from $L$ and insert $e$ into $Q$ with priority
$e$ itself. In other words, each element $e$ is its own priority. This
first phase of the sorting algorithm requires $n$ element extractions
from $L$ and $n$ element insertions into $Q$. The second phase of the
