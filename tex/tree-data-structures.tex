...
\item\label{item:tree_data_structures:height_balanced}
  ...
  rooted at $u$, the parent of $v$. If $S$ is a complete binary tree,
  then no height adjustment need to take place for vertices in $P_u$,
  hence $T_v$ is an AVL tree. Otherwise we perform any necessary
  height adjustment for vertices in $P_u$, starting from $u_k = u$ and
  working our way up to the root $r = u_0$. After adjusting the height
  of $u_i$, we test to see whether $u_i$~(with its new height) is
  height-balanced. If each of the $u_i$ with their new heights are
  height-balanced, then $T_v$ is height-balanced.
