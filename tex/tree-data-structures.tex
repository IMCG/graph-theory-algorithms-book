...
parent of $v$. If $v$ has no children, we set
$\child(v) = \texttt{NULL}$. If $v$ is one of the roots $r_{k_i}$,
we set $\parent(v) = \texttt{NULL}$. And if $v$ is the right-most
child of its parent, then we set $\sibling(v) = \texttt{NULL}$.

The roots $r_{k_0}, r_{k_1}, \dots, r_{k_n}$ can be organized as a
linked list with two functions for accessing the next root and the
previous root. The root immediately following $r_{k_i}$ is denoted
