...
$h_r$ be the height of $v$'s right subtree. Then the subtree rooted at
$v$ is said to be \emph{height-balanced} if
$|h_\ell - h_r| \leq 1$. For each internal vertex $u$ of $T$, if the
subtree rooted at $u$ is height-balanced, then the whole tree $T$ is
height-balanced.
\end{definition}
...
our problem is not yet over; $T_v$ is not height-balanced. To complete
the insertion procedure, we require a technique to restore the
height-balance property to $T_v$.
