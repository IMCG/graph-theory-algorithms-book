...
binary\index{binary search trees} search trees as a general data
...
%%
\begin{description}
\item[Heap-order property:]\index{binomial heap!order property}
...
%%
\begin{itemize}
\item \textbf{Left subtree property.} The left subtree of $v$ contains
  only vertices whose keys are at most $\kappa_v$. That is, if $u$ is
  a vertex in the left subtree of $v$, then $\kappa_u \leq \kappa_v$.

\item \textbf{Right subtree property.} The right subtree of $v$
  contains only vertices whose keys are at least $\kappa_v$. In other
  words, any vertex $u$ in the right subtree of $v$ satisfies
  $\kappa_v \leq \kappa_u$.

\item \textbf{Recursion property.} Both the left and right subtrees of
  $v$ must also be binary search trees.
\end{itemize}
%%
