...
A \emph{rooted tree}\index{tree!rooted} is a tree\index{tree} with a
specified \emph{root}\index{root} vertex $v_0$, i.e. the vertex $v_0$
...
Unix\index{Unix}, in particular Linux\index{Linux},
...
\begin{example}
Let $T = (V, E)$ be a binary tree with vertex set
\[
V
=
...
\]
Suppose that $x$ is the root of the branch we want to remove from
$T$. Find all children of $x$ and cut off the branch rooted at $x$
from $T$. Verify that the resulting graph is also a binary tree.
\end{example}
...
\end{lstlisting}
%%
The latter technique for binary tree testing is inefficient because in
the worst-case it scans through all the vertices of a graph. If $T$ is
a tree, we can test whether or not it is a binary tree as
follows. Each vertex in a binary tree has a maximum degree of $3$ and
the root vertex is the only vertex with degree $2$.
%%
\begin{lstlisting}
...
\begin{theorem}
\label{thm:trees_forests:basic_characterization_of_trees}
...
\begin{proof}
(1) $\implies$ (2):
This basically follows by induction on the number of vertices. By
definition, a tree has no cycles. Make the following induction
hypothesis: for any tree $T = (V,E)$, let $|E| = |V| - 1$. This holds
in the base case where $|V|=1$ since in that case, there can be no
edges. Assume it is true for all trees with $|V| = k$, for some
$k > 1$. Let $T = (V,E)$ be a tree having $k + 1$ vertices. Remove an
edge (but not the vertices it is incident to). This disconnects $T$
into $T_1 = (V_1, E_1)$ union $T_2 = (V_2, E_2)$, where
$|E| = |E_1| + |E_2| + 1$ and $|V| = |V_1| + |V_2|$ (and possibly one
of the $E_i$ is empty), each of which is a tree satisfying the
...
$k$. By part~(2), each of these satisfy
\[
|E_i|
=
|V_i| - 1
...
part~(3). Therefore $e$ is a cut set.
...
and
\[
P' : v'_0 = u,\, v'_1,\, v'_2, \dots, v'_\ell = v
\]
be two paths from $u$ to $v$.

(5) $\implies$ (6):
Let $e = (u,v)$ be a new edge connecting $u,v \in V$. Suppose that
\[
...
\]
are two cycles in $T \cup \big(\{u,v\},\, \{e\}\big)$.

If either $P$ or $P'$ does not contain $e$, say $P$ does not contain
$e$, then $P$ is a cycle in $T$. Let $u = v_0$ and let $v = v_1$. The
edge $v_0 = w v_1$ is a $u$-$v$ path and the sequence
$v = v_1,\, v_2, \dots, v_k = w = u$ taken in reverse order is another
$u$-$v$ path. This contradicts part~(5).

We may suppose now that $P$ and $P'$ both contain $e$. Therefore, $P$
...
as $P'$ except it lacks the edge from $u$ to $v$. By (5), these
$u$-$v$ paths $p_0$ and $P_0'$ must be the same. This forces $P$ and
$P'$ to be the same, which proves part~(6).
...
which is a tree\index{tree}. As in part~(6) of
Theorem~\ref{thm:trees_forests:basic_characterization_of_trees}, we
...
\begin{figure}[!htbp]
\centering
\input{image/trees-forests/Prim-algorithm-digraph2.tex}
...
$(x,y)$, where $x$ is an integer $j \in \Z_{256}$
