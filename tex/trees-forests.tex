...
trees\index{tree}, their properties, and various applications.
...
\emph{endpoint}\index{vertex!endpoint} or a \emph{leaf}\index{vertex!leaf}. Any
vertex that is not a leaf\index{vertex!leaf} is called an \emph{internal}
vertex. A connected forest\index{forest} is a
\emph{tree}\index{tree}. In other words, a tree\index{tree} is a graph
without cycles\index{cycle} and each edge is a bridge\index{bridge}.
...
circuit\index{electronic circuit} connecting several components. If
...
$O(m \cdot \log n)$. With a Fibonacci\index{heap!Fibonacci} heap
...
is an algorithm for finding a minimum\index{spanning tree!minimum}
...
``string''\index{string} into a sequence of binary\index{bits}
...
path on the hypercube\index{graph!hypercube} graph $Q_n$ that visits
...
over the binary alphabet is a finite sequence of $0$'s\index{bits} and
...
$r$. To determine the actual encoding of each symbol in the alphabet,
we feed $T$ and $r$ to
...
\item The following result was first published in
  Wiener\index{Wiener!Harry}~\cite{Wiener1947}. Let $T = (V,E)$ be a
