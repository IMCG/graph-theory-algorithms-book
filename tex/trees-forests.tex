...
$G = (V,E)$. The algorithm was developed in 1930 by Czech
mathematician V.~Jarn\'ik~\cite{Jarnik1930} and later independently by
R.~C.~Prim~\cite{Prim1957} and
E.~W.~Dijkstra~\cite{Dijkstra1959}. However, Prim was the first to
present an implementation that runs in time $O(n^2)$, where
$n = |V|$. Using $2$-heaps, the runtime can be
reduced~\cite{KershenbaumVanSlyke1972} to $O(m \cdot \log n)$, where
$m = |E|$. With a Fibonacci heap
