...
a rooted tree, we can test whether or not it is a binary tree as
follows. Each vertex in a binary tree has a maximum degree of $3$. If
the root vertex has exactly two children, then the root vertex is the
only vertex with degree
$2$. (Problem~\thechapter.\ref{prob:trees_forests:binary_tree_test}
asks you to prove this result.)
%%
\begin{lstlisting}
...
\item\label{prob:trees_forests:binary_tree_test} The solution of
  ...
  degree $2$, then $T$ is a binary tree. Prove this statement.
