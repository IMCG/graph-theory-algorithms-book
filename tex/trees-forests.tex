...
In general, the following recursive and constructive definition is an
alternative way to define trees. We say \emph{constructive} because it
provides an algorithm to contruct a tree, as opposed to the
...
\begin{definition}
\label{def:trees_forests:recursive_construction_trees}
\rm
...
\]
The recursive construction of $T$ is illustrated in
...
\item Discuss balanced trees. See section~6.2.3 in
  Knuth~\cite{Knuth1998} and chapter~15 in Sedgewick~\cite{Sedgewick1990}.
\end{itemize}
}
...
\item Definition~\ref{def:trees_forests:recursive_construction_trees}
  shows how to recursively construct a new tree from a given
  collection of trees, hence it is referred to as a recursive
  definition. To prove theorems based upon recursive definitions, we
  use a proof technique called \emph{structural induction}. Let $S(C)$
  ...
  for the basis structure(s) $C$. For the inductive case, let $X$ be a
  structure formed using the recursive definition from the structures
  ...
  constructed using
  Definition~\ref{def:trees_forests:recursive_construction_trees} is
  indeed a tree.
