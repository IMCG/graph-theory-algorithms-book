...
selected with equal probability. To generate a graph in $G(n,N)$,
start with $G$ being a graph on $n$ vertices but no edges. Then choose
$N$ of the possible $\binom{n}{2}$ edges independently and uniformly
at random and let the chosen edges be the edge set of $G$. Each graph
$G \in G(n,N)$ is associated with a probability
\[
