...
\begin{figure}[!htbp]
\centering
\input{image/random-graphs/Gnp-simulation.tex}
\caption{Comparison of expected and experimental values of the number
  ...
  vertical axis represent the size and total degree~(expected or
  ...
  generating $M = 500$ random graphs from $\cG(n, p_i)$. We compute the
  expected number of edges $\alpha$ and expected total degree $\beta$
  of any graph in $\cG(n, p_i)$. For each $G_i \in \cG(n, p_i)$, where
  $i = 1, 2, \dots, 500$, compute its actual size $\alpha_i$ and
  actual total degree $\beta_i$. Then take the mean $\hat{\alpha}$ of
  the $\alpha_i$ and the mean $\hat{\beta}$ of the $\beta_i$.}
...
\centering
\index{degree distribution}
\input{image/random-graphs/power-law-degree-distribution-simulation.tex}
