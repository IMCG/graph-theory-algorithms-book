...
the vertex set is $V = \{0, 1, \dots, n-1\}$. For each distinct pair
of vertices $i,j \in V$, let $d_{ij}$ be the distance from $i$ to
$j$. If there is no path from $i$ to $j$, set $d_{ij} = 0$. Thus
\[
d_{ij}
=
\begin{cases}
...
&=
\frac{2}{n(n-1)} \cdot \frac{1}{2} \sum_{i \neq j} d_{ij} \\[4pt]
&=
\frac{1}{n(n-1)} \sum_{i \neq j} d_{ij}
\end{align*}
%%
