...
\subsubsection{Binomial random graph model}
...
to our needs or search for a more efficient algorithm.
Figure~\ref{fig:random_graphs:binomial_random_graph_25_nodes}
illustrates some graphs from $G(25,p)$ with $p = i/6$ for
$i = 0, 1, \dots, 5$. The expected number of edges of any
$G \in G(n,p)$ is
\[
\alpha
=
...
\item Algorithm~\ref{alg:random_graphs:generate_random_Gnp} can be
  ...
  %%
  \begin{enumerate}[(a)]
  \item Consider a vertex set $V = \{1, 2, \dots, n\}$ with $n > 1$
    and let $E$ be the set of all $2$-combinations of $V$, where each
    $2$-combination is written $ij$. Show that $ij \in E$ if and only
    if $i < j$.

  \item From the previous exercise, we know that if $1 \leq i < n$
    then there are $n - i$ pairs $jk$ where either $i = j$ or
    $i = k$. Show that
    \[
    \sum_{i=1}^{n-1} (n - i)
    =
