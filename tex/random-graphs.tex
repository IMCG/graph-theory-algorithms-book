...
of a collection of graphs having some common properties. Recall that
...
\index{acyclic!graph!random}
\index{algorithm!random}
\index{directed graph!random}
\index{simple graph!random}
\index{weight!graph!random}
\input{algorithm/random-graphs/random-graph.tex}
...
Fix a positive integer $n$ and a probability $p$. Two closely related
models of random graphs are $G_{n,p}$ and $G_{n,N}$. The model
$G_{n,p}$ considers the collection of all undirected graphs on $n$
vertices, each such graph having at most $\binom{n}{2}$ edges, $m$
actual edges, and an associated probability
\[
p^m (1 - p)^{\binom{n}{2} - m}.
\]
Notice the resemblance to the binomial\index{binomial distribution}
distribution. To generate a random graph in $G_{n,p}$, start with $G$
being the null\index{null graph} graph. Consider each of the
$\binom{n}{2}$ possible edges in some order and add it independently
to $G$ with probability $p$. The expected number of edges in the
resulting graph is
\[
p \cdot \binom{n}{2}
=
