...
Suppose we want to generate a random number from a geometric
distribution, i.e. we want to simulate $X$ such that
\[
\Pr[X = k]
=
p (1 - p)^{k-1},
...
1 - (1 - p)^{\ell - 1}.
\]
In other words, we can simulate a geometric random variable by
...
(1 - p)^k < 1 - r < (1 - p)^{k-1}
\]
where $1 - r$ and $r$ are both uniformly distributed. Thus we can
define $X$ by
%%
\begin{align*}
X
...
The \emph{clustering coefficient}\index{clustering coefficient} of a
...
coefficient\index{clustering coefficient} of $G$ is defined by
\[
C(G)
=
