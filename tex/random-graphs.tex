...
Batagelj and Brandes\index{Batagelj-Brandes algorithm} show that we
can choose $k$ to be
\[
k
=
...
a paper makes a reference to the Erd\H{o}s-R\'enyi model, one should
question whether the paper is referring to $\cG(n,p)$ or $\cG(n,N)$.
...
%%
\begin{itemize}
\item social: collaboration network of actors in feature
  films~\cite{AmaralEtAl2000,WattsStrogatz1998}, scientific
  publication
  authorship~\cite{CastroGrossman1999,GrossmanIon1995,Newman2001a,Newman2001b};

\item information: citation network~\cite{Redner1998}, Roget's
  Thesaurus~\cite{Knuth1993}, word
  co-occurrence~\cite{DorogovtsevMendes2001,FerrerSole2001};

\item technological: internet~\cite{ChenEtAl2002,FaloutsosEtAl1999},
  power grid~\cite{WattsStrogatz1998}, train
  routes~\cite{SenEtAl2003},
  software~\cite{Newman2003a,ValverdeEtAl2002};

\item biological: metabolic network~\cite{JeongEtAl2000}, protein
...
Watts and Strogatz~\cite{Watts1999a,Watts1999b,WattsStrogatz1998}
...
$G$ has vertex set $V = \{0, 1, \dots, n-1\}$ and make the assignment
$v \assign -1$. If $r$ is chosen uniformly at random from $[0,1)$,
then the vertex to be rewired is given by
\[
...
and $m$ is the size of the $k$-circulant graph on $n$ vertices.
