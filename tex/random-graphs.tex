...
\]
where $N_i$ counts the number of edges in $\cN_i$. In case $i$ is an
isolated vertex, i.e. $\deg(i) = 0$, we set the local clustering
coefficient of $i$ to be zero. Then the
clustering coefficient\index{clustering coefficient} $C(G)$
of $G$ is defined by
\[
C(G)
=
