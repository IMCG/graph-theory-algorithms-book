...
undirected graphs on $n$ vertices. If $G$ is any element of the
probability space $G(n,p)$ and $ij$ is any edge, then $ij$ occurs as
an edge of $G$ independently with probability $p$. In symbols, for any
$i,j \in V$ we have
\[
\Pr[ij \in E(G)]
=
...
probability $p$. The expected number of edges in the resulting graph
is
\[
p \cdot \binom{n}{2}
=
...
we know that the number of simple graphs on $n$ vertices is given by
\[
2^{n(n-1) / 2}
\]
where the probability of any of these graphs being the output of the
above procedure is given
by~\eqref{eqn:random_graphs:probability_of_chosen_graph_binomial_model}.
